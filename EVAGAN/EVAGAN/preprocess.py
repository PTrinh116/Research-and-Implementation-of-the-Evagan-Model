...
def prepare_ISCX_2014_data(PATH="", INPUT_FILE_NAME=""):
    ...
    if network_traffic:
        if("SrcIP" in data.columns):
          data.rename(columns={'SrcIP': 'Src IP','SrcPort': 'Src Port', 'DstIP': 'Dst IP', 'DstPort': 'Dst Port', 'FlowID': 'Flow ID', "FlowIATStd": "Flow IAT Std", "FwdIATStd": "Fwd IAT Std", 'CWRFlagCount': 'CWR Flag Count' }, inplace=True)
        '''
        if(INPUT_FILE_NAME == r'ISCX_Botnet-Testing.pcap_Flow.csv'):
           data = data.drop(
    ...
    inf_indx = data_df.index[np.isinf(data_df).any(axis=1)]
    ...
    inf_indx = data.index[np.isinf(data_df).any(axis=1)]

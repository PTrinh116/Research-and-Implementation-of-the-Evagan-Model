    ...
    def train(self, model_components):
        ...
        for i in range(starting_step, starting_step + nb_steps):
            ...
            for j in range(k_d):
                ...
                t_b = get_data_batch(
                    shuffled_Bots[0 : int(shuffled_Bots.shape[0] * 0.7)],
                    batch_size,
                    seed=i + j,
                )
                T_b = get_data_batch(
                    shuffled_Normal[0 : int(shuffled_Normal.shape[0] * 0.7)],
                    batch_size,
                    seed=i + j,
                )
                t_b = pd.DataFrame(t_b, columns=shuffled_Bots.columns)
                labels = t_b["Label"]

                # print("<><><><><><><><><><><><><><><><><><><>")
                # print(labels.value_counts())
                # print("<><><><><><><><><><><><><><><><><><><>")
                T_b = pd.DataFrame(T_b, columns=shuffled_Normal.columns)
                Labels = T_b["Label"]
                # LABELS = T_B['Label']
        ...
        GEN_VALIDITY = pd.DataFrame(Disc_G_Z_Real_acc_, columns=["GEN_VALIDITY"])
        FAKE_BOT_EVA = pd.DataFrame(Disc_G_Z_B_acc_, columns=["FAKE_BOT_EVA"])
        REAL_NORMAL_EST = pd.DataFrame(Disc_N_acc_, columns=["REAL_NORMAL_EST"])
        REAL_BOT_EVA = pd.DataFrame(Disc_B_acc_, columns=["REAL_BOT_EVA"])

        D_Loss_Real_Bot = pd.DataFrame(list_loss_real_bot, columns=["D_Loss_Real_Bot"])
        D_Loss_Fake_Bot = pd.DataFrame(list_loss_fake_bot, columns=["D_Loss_Fake_Bot"])

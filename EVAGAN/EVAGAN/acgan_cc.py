...
class ACGAN_CC:
    ...
    def train(self, model_components):
        ...
        t_b = Bots[0 : int(Bots.shape[0] * 0.7)]
        T_n = Normal[0 : int(Normal.shape[0] * 0.7)]
        print("T_n: " + str(T_n.shape))

        T = pd.concat([t_b, T_n]).reset_index(drop=True)  # Augmenting with real botnets
        ...
        for i in range(starting_step, starting_step + nb_steps):
            for j in range(k_d):
                ...
                if USE_UNIFORM_NOISE:
                    z = np.random.uniform(mean, stdv, (batch_size, rand_noise_dim))
                # shuffled_T_B_batch = get_data_batch(shuffled_T_B[0:int(shuffled_T_B.shape[0]*.7)], batch_size, seed=i+j)
                shuffled_T_B_batch = get_data_batch(
                    shuffled_T_B, batch_size, seed=i + j
                )
                shuffled_T_B_batch=pd.DataFrame(shuffled_T_B_batch, columns=shuffled_T_B.columns)
            ...
            for j in range(k_g):
                ...
                g_loss = self.combined.train_on_batch(
                    [z, sampled_labels], [real, sampled_labels]
                )
            ...
            if not i % log_interval:  # 2x faster than testing each step...
                ...
                z = np.random.normal(size=(Test_N_or_B.shape[0], rand_noise_dim))
                if USE_UNIFORM_NOISE:
                    z = np.random.uniform(size=(Test_N_or_B.shape[0], rand_noise_dim))
                Sampled_labels = np.random.randint(0, 2, (Test_N_or_B.shape[0], 1))
                G_z = self.generator.predict([z, Sampled_labels])
        ...
        if ESTIMATE_CLASSIFIERS:
            ...
            pred_total_XGB = pd.DataFrame(pred_total_XGB, columns=["pred_total_XGB"])
            pred_total_DT = pd.DataFrame(pred_total_DT, columns=["pred_total_DT"])
            pred_total_NB = pd.DataFrame(pred_total_NB, columns=["pred_total_NB"])
            pred_total_RF = pd.DataFrame(pred_total_RF, columns=["pred_total_RF"])
            pred_total_LR = pd.DataFrame(pred_total_LR, columns=["pred_total_LR"])
            pred_total_KNN = pd.DataFrame(pred_total_KNN, columns=["pred_total_KNN"])

            pred_G_Z_XGB = pd.DataFrame(pred_G_Z_XGB, columns=["pred_G_Z_XGB"])
            ...
            pred_G_Z_KNN = pd.DataFrame(pred_G_Z_KNN, columns=["pred_G_Z_KNN"])
